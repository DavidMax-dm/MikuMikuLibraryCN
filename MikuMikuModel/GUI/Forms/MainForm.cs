...
public partial class MainForm : Form
{
    ...
    {
        // 假设 mMenuStrip 是主菜单
        var addModelMenuItem = new ToolStripMenuItem("添加模型");
        addModelMenuItem.Click += OnAddModel;
        mMenuStrip.Items.Add(addModelMenuItem);
    }
        ...
        using (var dialog = new OpenFileDialog())
        {
            ...
            dialog.Filter = ModuleFilterGenerator.GenerateFilter(
                FormatModuleRegistry.ModelTypes.Where(x =>
                    typeof(IBinaryFile).IsAssignableFrom(x) && x.IsClass && !x.IsAbstract && NodeFactory.NodeTypes.ContainsKey(x)),
                FormatExtensionFlags.Import);
            dialog.Title = "选择要添加的模型文件.";
            dialog.ValidateNames = true;
            dialog.AddExtension = true;

            if (dialog.ShowDialog() == DialogResult.OK)
            {
                AddModel(dialog.FileName);
            }
        }
    }
    ...
    private void AddModel(string filePath)
    {
        ...
        try
        {
            ...
            if (node != null && !typeof(IBinaryFile).IsAssignableFrom(node.DataType))
                exceptionMessage = "文件类型无法确定.";

            if (node == null || !string.IsNullOrEmpty(exceptionMessage))
            {
                MessageBox.Show($"无法添加 {Path.GetFileName(filePath)}.\n原因: {exceptionMessage}", Program.Name, MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
